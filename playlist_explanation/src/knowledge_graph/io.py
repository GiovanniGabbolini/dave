...
def save_sub_graphs(l, start_from_batch=0, folder_name="sub_graphs_interestingness"):
    ...
    # We build a graph with every feature but the lyrics
    class _RestrictedActionsSupplier(ActionsSupplier):

        def eligible_actions(self):
            exclueded_actions = ['track_lyrics']
            all_actions = specific.__all__ + common.__all__
            return list(set(all_actions) - set(exclueded_actions))
    ...
    for idx, d in tqdm(enumerate(l)):
        ...
        if batch_n >= start_from_batch:
            ...
            g = construct_graph(d, supplier=_RestrictedActionsSupplier())
