...
from src.features import specific, common
...
class ActionsSupplier:
    ...
    def eligible_actions(self):
        return specific.__all__ + common.__all__
        # return ['token_phrase', 'word_phonetics', 'word_stem', 'word']
    ...
    def applicable_actions(self, g):
        ...
        for f in functions:
            ...
            l += self.applicable_actions_given_function(g, func, False)
        ...
        if len(l) > 0:
            ...
            for f in functions:
                # Get the argument variables name for the function f
                func = getattr(globals()[f], f)
                l += self.applicable_actions_given_function(g, func, True)
            return l

    def applicable_actions_given_function(self, g, func, allow_default_values):
        ...
        Arguments:
            g {nx graph} -- Graph handling the knowledge acquired so far
        ...
        for arg, v in sig.parameters.items():
            ...
            for k in g.nodes():
                ...
                if node['type'] in eligible_types_arg:
                    ...
                    if self.eligible_nodes_filter(g, node, func):
...
class MockedActionsSupplier():
    ...
    def __init__(self):
        self.actions_supplier = ActionsSupplier()

    def applicable_actions(self, g):
        actions = self.actions_supplier.applicable_actions(g)
