...
def area_country(area_musicbrainz_id) -> 'country_musicbrainz':
    """Given an area id, returns the country associated with that area

    Returns:
        str -- 
    """
    country_id = look_backward_recursive(area_musicbrainz_id['value'])
...
def look_backward_recursive(id_area):
    ...
    if area_type == 'Country':
        ...
        if len(rels_backward) > 0:
            return look_backward_recursive(rels_backward[0]['target'])
        else:
            return None
